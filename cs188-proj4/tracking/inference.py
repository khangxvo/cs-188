...
def constructBayesNet(gameState: hunters.GameState):
    ...
    # Get all possible position of PAC and GHOSTs
    # Calculate the possible vals for OBS
    possible_pos = []
    for x in range(X_RANGE): 
        for y in range(Y_RANGE):
            possible_pos.append((x, y))              

    # Assign PAC and GHOST their possible position
    variableDomainsDict[PAC] = possible_pos
    variableDomainsDict[GHOST0] = possible_pos
    variableDomainsDict[GHOST1] = possible_pos

    # Calculate the possible distance for OBS
    possible_dis = list(range(X_RANGE - 1 + Y_RANGE - 1 + MAX_NOISE + 1))

    # Assign OBSs their possible distance
    variableDomainsDict[OBS0] = possible_dis
    variableDomainsDict[OBS1] = possible_dis
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # get only the factors that corresponding the the evidence (reduce the size)
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        
        # start performing join and elimination alternatively
        # according the eliminationOrder List
        for var in eliminationOrder:
            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, var)

            # if the joinedFactor only have 1 uncondition var, eliminate it would produce 1,
            # so just skip it
            if len(joinedFactor.unconditionedVariables()) > 1:
                
                afterEliminationjoinedFactor = eliminate(joinedFactor, var)
                currentFactorsList.append(afterEliminationjoinedFactor)

        # join all factors
        fullJoint = joinFactors(currentFactorsList)

        # normalize it
        returnFactor = normalize(fullJoint)
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        key_val_list = self.items()
        total_val = self.total()
        if len(key_val_list) == 0 or total_val == 0:
            return
        
        for key, val in self.items():
            self[key] = val/total_val
    ...
    def sample(self):
        ...
        # raiseNotDefined()
        total_val = self.total()
        random_val = random.uniform(0, total_val)

        accumProb = 0
        for key, val in self.items():
            if accumProb + val > random_val:
                return key
            accumProb += val
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        updateBelief = DiscreteDistribution()

        for potentialPos in self.allPositions:
            updateBelief[potentialPos] = curBelief[potentialPos] * self.getObservationProb(observation, pacmanPos, potentialPos, jailPos)
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in curBelief:
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            for pos, prob in newPosDist.items():
                newBelief[pos] += curBelief[oldPos] * prob
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for i in range(self.numParticles):
            ...
            action = self.legalPositions[i % legalLen]
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for p in self.particles:
            weight[p] += self.getObservationProb(observation, pacPos, p, jailPos)
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            posDistribution = self.getPositionDistribution(gameState, oldPos)
            sampleVal = posDistribution.sample()
